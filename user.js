...
// আপনার ক্লাউডফ্লেয়ার ওয়ার্কারের আসল URL দিন
const CLOUDFLARE_WORKER_URL = "https://polished-disk-2b4e.tirtharoyvuson.workers.dev/"; 
...
let appState = {
    ...
    matches: [],
    esportsData: {
        matches: [], registrations: {}, counts: {}, points: [], history: []
    },
    myMatches: [],
    results: [],
    tasks: [], 
...
async function secureFirebaseLogin() {
    try {
        ...
        if (!initData) {
            console.warn("টেলিগ্রাম এনভায়রনমেন্ট পাওয়া যায়নি। ডেমো মোডে চলছে।");
        ...
        const response = await fetch(CLOUDFLARE_WORKER_URL, {
            ...
            body: JSON.stringify({ 
                initData: initData, 
                action: "login" // এটি আপনার আগের কোডে মিসিং ছিল!
            })
        });

        const data = await response.json();

        // সার্ভার থেকে কোনো এরর আসলে সেটি ধরবে
        if (!response.ok) {
        ...
        // এখন আসল সমস্যাটি অ্যালার্টে দেখতে পাবেন
